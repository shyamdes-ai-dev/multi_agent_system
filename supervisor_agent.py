...
def create_supervisor_system():
    ...
    class RouteDecision(BaseModel):
        next: Literal["researcher", "writer", "critic", "FINISH"] = Field(
            description="The next agent to call, or FINISH if task ifs complete"
        )
    ...
    def researcher(state: SuperVisorState) -> dict:
        ...
        return {
            "messages": [
                HumanMessage(content=f"[Researcher]: {response.content[0].get('text')}")
            ]
        }
    ...
    def writer(state: SuperVisorState) -> dict:
        ...
        return {
            "messages": [
                HumanMessage(content=f"[Writer]: {response.content[0].get('text')}")
            ]
        }
    ...
    def critic(state: SuperVisorState) -> dict:
        ...
        return {
            "messages": [
                HumanMessage(content=f"[Critic]: {response.content[0].get('text')}")
            ]
        }
